...
async function seedRandomMatches() {
  ...
  try {
    ...
    // Get users and courts
    const users = await userRepository.find({ take: 50 });
    const courts = await courtRepository.find({ take: 50, withDeleted: false });

    if (users.length < 5) {
      console.error('❌ Need at least 5 users. Please seed users first.');
      process.exit(1);
    }

    if (courts.length === 0) {
      console.error('❌ No courts found. Please add courts first.');
      process.exit(1);
    }

    console.log(`📊 Found ${users.length} users and ${courts.length} courts`);
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const selectedTimeSlots: typeof timeSlots = [];
